// ====================================================
// نظام إدارة الزيت والجبنة - Google Sheets Backend
// ====================================================
...
async function loadAllData() {
  ...
  for (const [sheet, key] of map) {
    ...
    if (r.success && Array.isArray(r.data)) {
      state[key] = r.data.map(row => sanitizeRow(row));
    }
  }
  refreshUsersDL();
...
// تنظيف الصف القادم من Sheets وتحويل الأرقام
function sanitizeRow(row) {
  ...
  for (const [k, v] of Object.entries(row)) {
    ...
    // حقول رقمية معروفة
    const numFields = ['cashCount','instCount','cashPrice','instPrice',
                       'tanksCount','amount','amountPaid','totalAmount','paid','total'];
...
function showModal(title, bodyHtml, onConfirm, confirmLabel) {
  ...
  if (!overlay) return;
  document.getElementById('modalTitle').textContent = String(title);
...
function refreshUsersDL() {
  ...
  if (!dl) return;
  dl.innerHTML = state.users
    .map(u => `<option value="${safeStr(u.name)}">[${safeStr(u.userId)}]</option>`)
    .join('');
}

function fillUserPrices(prefix) {
  const el = document.getElementById(prefix + '_nameId');
  if (!el) return;
  const u = findUser(el.value);
  if (!u) return;
...
function getTotalPurchased() {
  return state.purchases.reduce((s, b) => s + (parseFloat(b.tanksCount) || 0), 0);
}
function getTotalDistributed() {
  return state.tankRecords.reduce((s, r) =>
    s + (parseFloat(r.cashCount) || 0) + (parseFloat(r.instCount) || 0), 0);
}
...
function refreshBatchDropdown() {
  ...
  sel.innerHTML = '<option value="">-- اختر دفعة --</option>' +
    batches.map(b =>
      `<option value="${safeStr(b.batchNumber)}">دفعة ${safeStr(b.batchNumber)} (${b.tanksCount} تنك)</option>`
    ).join('');
}
...
function calcCash(userId) {
  if (!userId) return { totalCashTanks:0, avgCashPrice:0, totalCashDue:0, totalCashPaid:0, cashTanksPaid:0, cashTanksRemaining:0, cashMoneyRemaining:0 };
  const userTanks      = state.tankRecords.filter(r => r.userId === userId);
  const totalCashTanks = userTanks.reduce((s, r) => s + (parseFloat(r.cashCount) || 0), 0);
  const totalCashDue   = userTanks.reduce((s, r) =>
    s + ((parseFloat(r.cashCount) || 0) * (parseFloat(r.cashPrice) || 0)), 0);
...
function calcInstallments(userId) {
  if (!userId) return { totalInstTanks:0, avgInstPrice:0, totalInstDue:0, totalInstPaid:0, instTanksPaid:0, instTanksRemaining:0, instMoneyRemaining:0 };
  const userTanks      = state.tankRecords.filter(r => r.userId === userId);
  const totalInstTanks = userTanks.reduce((s, r) => s + (parseFloat(r.instCount) || 0), 0);
  const totalInstDue   = userTanks.reduce((s, r) =>
    s + ((parseFloat(r.instCount) || 0) * (parseFloat(r.instPrice) || 0)), 0);
...
async function addTankRecord() {
  ...
  const cashCount = parseFloat(document.getElementById('tk_cash').value) || 0;
  const cashPrice = parseFloat(document.getElementById('tk_cashPrice').value) || 0;
  const instCount = parseFloat(document.getElementById('tk_inst').value) || 0;
  ...
  if (cashCount + instCount <= 0)   { showToast('أدخل عدد تنكات صحيح', 'error'); return; }
  if (cashCount > 0 && cashPrice <= 0) { showToast('أدخل سعر تنك الكاش', 'error'); return; }
  if (instCount > 0 && instPrice <= 0) { showToast('أدخل سعر تنك الأقساط', 'error'); return; }

  const total = cashCount + instCount;
  const stock = getStock();
  if (total > stock) {
    showToast(`❌ المخزن يحتوي فقط ${stock} تنك — لا يمكن تسجيل ${total}`, 'error');
    return;
  }
  ...
  if (r.success) {
    ...
    renderAll();
    showToast(`تم تسجيل ${total} تنك ✓`, 'success');
  } else {
...
async function addInstallment() {
  ...
  if (r.success) {
    ...
    renderAll();
    showToast('تم حفظ القسط ✓', 'success');
  } else {
    showToast('فشل الحفظ: ' + (r.error || ''), 'error');
  }
}
...
async function addCashPayment() {
  ...
  if (r.success) {
    ...
    renderAll();
    showToast('تم حفظ دفعة الكاش ✓', 'success');
  } else {
    showToast('فشل الحفظ: ' + (r.error || ''), 'error');
  }
}
...
async function addPurchase() {
  ...
  const count    = parseFloat(document.getElementById('pur_count').value);
  const notes    = document.getElementById('pur_notes').value.trim();

  if (!supplier)                         { showToast('أدخل اسم المورد', 'error'); return; }
  if (!batchNum || isNaN(parseInt(batchNum))) { showToast('أدخل رقم دفعة صحيح', 'error'); return; }
  ...
  if (state.purchases.find(b => String(b.batchNumber) === String(batchNum))) {
    showToast(`❌ دفعة ${batchNum} موجودة مسبقاً`, 'error'); return;
  }
  ...
  if (r.success) {
    ...
    refreshBatchDropdown();
    renderAll();
    showToast(`تم تسجيل دفعة ${batchNum} (${count} تنك) ✓`, 'success');
  } else {
    showToast('فشل الحفظ: ' + (r.error || ''), 'error');
  }
}
...
async function addCheese() {
  ...
  if (isNaN(paid) || paid < 0)  { showToast('أدخل المبلغ المدفوع', 'error'); return; }
  ...
  if (r.success) {
    ...
    showToast(`تم الحفظ. متبقي: ${fmtMoney(rem)} د.أ`, rem > total * 0.5 ? 'warning' : 'success');
    if (rem > total * 0.5) showToast('⚠️ المتبقي أكبر من 50%!', 'warning');
  } else {
    showToast('فشل الحفظ: ' + (r.error || ''), 'error');
  }
}
...
function askDelete(sheetName, transactionId, stateKey) {
  showModal('🗑️ تأكيد الحذف',
    ...
    async () => {
      const passEl = document.getElementById('delPassConfirm');
      if (!passEl) return;
      const pass = passEl.value;
      if (pass !== state.deletePassword) { showToast('كلمة سر خاطئة', 'error'); return; }
      ...
      if (r.success) {
        ...
        showToast('فشل الحذف: ' + (r.error || ''), 'error');
      }
    }, '🗑️ احذف');
}
...
function renderTanks() {
  ...
  tbody.innerHTML = !recs.length
    ...
    : recs.map(r => `<tr>
        ...
        <td><strong>${safeStr(r.userName)}</strong><br>
            <small style="color:var(--text3)">${safeStr(r.userId)}</small></td>
        <td>دفعة ${safeStr(r.batchNumber)}</td>
        <td style="color:var(--gold);font-weight:700">${parseFloat(r.cashCount) || 0}</td>
        <td>${fmtMoney(r.cashPrice)} د.أ</td>
        <td style="color:var(--purple);font-weight:700">${parseFloat(r.instCount) || 0}</td>
  ...
  state.tankRecords.forEach(r => {
    const b = r.batchNumber || '?';
    if (!bsMap[b]) bsMap[b] = { cash: 0, inst: 0 };
    bsMap[b].cash += parseFloat(r.cashCount) || 0;
    bsMap[b].inst += parseFloat(r.instCount) || 0;
  });
  ...
  bs.innerHTML = entries.length
    ? entries.map(([b, v]) => `
        <div class="batch-card">
          <div class="batch-num">دفعة ${safeStr(b)}</div>
...
function renderPurchases() {
  ...
  tbody.innerHTML = !batches.length
    ...
    : batches.map(b => `<tr>
        <td><strong>دفعة ${safeStr(b.batchNumber)}</strong></td>
        <td>${safeStr(b.supplierName)}</td>
  ...
  state.tankRecords.forEach(r => {
    const b = r.batchNumber || '?';
    if (!distByBatch[b]) distByBatch[b] = { cash: 0, inst: 0 };
    distByBatch[b].cash += parseFloat(r.cashCount) || 0;
    distByBatch[b].inst += parseFloat(r.instCount) || 0;
  });

  const allBatches = new Set([...batches.map(b => b.batchNumber), ...Object.keys(distByBatch)]);
  const totalP = batches.reduce((s, b) => s + (parseFloat(b.tanksCount) || 0), 0);
  ...
  Array.from(allBatches).sort((a, b) => (parseInt(a) || 0) - (parseInt(b) || 0)).forEach(bNum => {
    ...
    const purchased = pur ? parseFloat(pur.tanksCount) || 0 : 0;
    ...
    html += `<div class="match-row match-${cls}">
      <div><strong>دفعة ${safeStr(bNum)}</strong></div>
...
function renderBank() {
  ...
  tbody.innerHTML = !recs.length
    ...
    : recs.map(r => `<tr>
        ...
        <td style="max-width:300px;word-break:break-word;font-size:12px">${safeStr(r.msg)}</td>
        <td>${fmtDate(r.createdAt)}</td>
        <td><button class="btn-icon"
...
function renderStats() {
  ...
  setEl('st_received',    received);
  setEl('st_distributed', distributed);
  setEl('st_stock',       stock);
  setEl('stockDisplay',   stock);
}
...
function doSearch() {
  ...
  res.innerHTML = matched.map(u => {
    const cc = calcCash(u.userId);
    const ci = calcInstallments(u.userId);
    ...
    return `<div class="card">
      <h3 class="card-title">👤 ${safeStr(u.name)}
        <code style="font-size:12px;color:var(--text3);font-weight:normal">ID: ${safeStr(u.userId)}</code>
      </h3>
      ...
      ${uTanks.length ? `
        ...
        <div class="table-wrapper"><table class="data-table">
          ...
          <tbody>${uTanks.map(r => `<tr>
            <td>دفعة ${safeStr(r.batchNumber)}</td>
            <td style="color:var(--gold)">${parseFloat(r.cashCount)||0}</td>
            <td>${fmtMoney(r.cashPrice)} د.أ</td>
            <td style="color:var(--purple)">${parseFloat(r.instCount)||0}</td>
            <td>${fmtMoney(r.instPrice)} د.أ</td>
            <td>${fmtDate(r.createdAt)}</td>
          </tr>`).join('')}</tbody>
        </table></div>` : ''}
      ...
      ${uCheese.length ? `
        ...
        <div class="table-wrapper"><table class="data-table">
          ...
          <tbody>${uCheese.map(c => {
            ...
            return `<tr>
              <td><code>${safeStr(c.batchNumber)}</code></td>
...
function checkLatePayments() {
  ...
  if ([1, 15, 25].includes(day)) {
    showToast(`📅 تذكير: اليوم ${day} — راجع المدفوعات`, 'warning');
  }

  if (day >= 25) {
    const late = [];
    state.users.forEach(u => {
      const hasInstTanks = state.tankRecords.some(r =>
        r.userId === u.userId && (parseFloat(r.instCount) || 0) > 0);
      if (!hasInstTanks) return;
      const paidThisMonth = state.installments.some(t =>
        t.userId === u.userId && t.monthPaid === currentMonth);
...
function exportCSV() {
  ...
  state.tankRecords.forEach(r =>
    rows.push(['تنك', r.transactionId, r.userName,
      `كاش:${r.cashCount||0} أقساط:${r.instCount||0}`, r.createdAt]));
  state.installments.forEach(r =>
    rows.push(['قسط', r.transactionId, r.userName, r.amountPaid, r.createdAt]));
  state.cashPayments.forEach(r =>
    rows.push(['كاش', r.transactionId, r.userName, r.amount, r.createdAt]));
  state.cheese.forEach(r =>
    rows.push(['جبنة', r.transactionId, r.userName,
      `مدفوع:${r.amountPaid} إجمالي:${r.totalAmount}`, r.createdAt]));
  state.otherTransfers.forEach(r =>
    rows.push(['تحويل', r.transactionId, r.description, r.amount, r.createdAt]));
...
function saveDelPass() {
  const el = document.getElementById('delPassInput');
  if (!el) return;
  const v = el.value;
...
function saveApiSettings() {
  const urlEl = document.getElementById('apiUrlInput');
